...
function displayBreedInfo(displayBreedData) {
	...
	// Add the breed info to the appropriate container
	breedDisplayEl.appendChild(img);
			...
			if (response.ok) {
				response.json().then(function(data) {
					...
					if (matchingBreeds.length === 0) {
						...
						displayModal(
							"Sorry, there aren't any breeds with all of those characteristics.  Please try again."
						);

						console.log('matchingBreeds is empty in MatchUsrTmps');
						return null;
					}
					...
					// After all breeds are appended, we reveal the select tool by replacing the class list that has "hide"
          // with the same class list without "hide"
					document.getElementById('breedChoice').setAttribute('class', 'input-field col s12');
